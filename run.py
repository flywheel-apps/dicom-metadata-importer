...
def get_session_label(dcm):
    ...
    if dcm.get('Manufacturer') and (dcm.get('Manufacturer').find('GE') != -1 or
                                    dcm.get('Manufacturer').find('Philips') != -1) and dcm.get('StudyID'):
...
def timestamp(date, time, timezone_input):
    ...
    if date and time and timezone_input:
        ...
        try:
            return timezone_input.localize(datetime.datetime.strptime(date + time[:6], '%Y%m%d%H%M%S'), timezone_input)
        except:
...
def get_timestamp(dcm, input_timezone):
    ...
    acquisition_timestamp = timestamp(acquitision_date, acquisition_time, input_timezone)
...
if __name__ == '__main__':
    ...
    metadatafile = import_metadata(dicom_filepath, output_filepath, timezone)
